...
| Run learned top-K block selection | `pipeline.py` with `unlearn.types_config.LORA.layer_selection_mode="learned_topk_hard"` | `unlearn_corpus.py`, `pipeline.py` | `conf/default.yaml` → `unlearn.types_config.LORA` |
...
```python
{
    ...
    "max_samples": int,
    "gate_metadata": dict,      # For learned top-K: {lora_layer_budget_k, selected_blocks, final_gate_scores, gate_tau_start, gate_tau_end, gate_seed}
}
```
...
**Gate Metadata (Learned Top-K):**
When `layer_selection_mode="learned_topk_hard"`, the `gate_metadata` dict contains:
- `lora_layer_budget_k`: Number of blocks selected (int)
- `selected_blocks`: Final canonical list of selected block indices (list[int])
- `final_gate_scores`: Final gate logit values (list[float])
- `gate_tau_start`: Initial temperature (float)
- `gate_tau_end`: Final temperature (float)
- `gate_seed`: Random seed used for gate initialization (int)
...
**Location:** 
- Standard: `models/{run_name}/{unlearn_type}/{dataset}/{wandb_project_name}/rank{lora_rank}-sc{steering_coeff}-{model_id}-rc{retain_coeff}-lr{lr}-epochs{epochs}/`
- With learned top-K: `models/{run_name}/{unlearn_type}/{dataset}/{wandb_project_name}/rank{lora_rank}-k{lora_layer_budget_k}-sc{steering_coeff}-{model_id}-rc{retain_coeff}-lr{lr}-epochs{epochs}/`

**Generated by:** `unlearn_corpus.main()` (called via `unlearn()` remote function)

**Code reference:** ```2806-2825:pipeline.py```
...
# Learned top-K (K=4 blocks selected)
models/2024-12-25_14-30-00/LORA/YEARS/my_experiment/rank8-k4-sc20-TinyLlama_TinyLlama-1.1B-Chat-v1.0-rc0.1-lr4e-07-epochs5/
```
...
### (e.1.1) Run Learned Top-K Block Selection (LoRA)

Run LoRA unlearning with learned Top-K block selection. During training, exactly K transformer blocks will have active LoRA adapters (selected via learned gate logits with Top-K hard mask and straight-through estimator). The checkpoint is hardened by zeroing non-selected LoRA weights before saving.

```bash
# Basic learned top-K (K=4 blocks)
python pipeline.py \
    datasets=[YEARS] \
    unlearn.types=[LORA] \
    unlearn.lora_ranks=[8] \
    unlearn.types_config.LORA.layer_selection_mode="learned_topk_hard" \
    unlearn.types_config.LORA.lora_layer_budget_k=4 \
    unlearn.types_config.LORA.gate_tau_start=10.0 \
    unlearn.types_config.LORA.gate_tau_end=0.1 \
    model_id="TinyLlama/TinyLlama-1.1B-Chat-v1.0" \
    wandb_project_name="topk_experiment" \
    dont_ft=true

# With matched forgetting (sweeps over K values)
python pipeline.py \
    datasets=[YEARS] \
    unlearn.types=[LORA] \
    unlearn.lora_ranks=[8] \
    unlearn.types_config.LORA.layer_selection_mode="learned_topk_hard" \
    unlearn.types_config.LORA.lora_layer_budget_k.range=[2,4,8] \
    unlearn.types_config.LORA.lora_layer_budget_k.add=[16] \
    matched_forgetting.enabled=true \
    model_id="Qwen/Qwen2.5-3B-Instruct"
```

**Key Features:**
- Exactly K blocks selected via learned gate logits
- Temperature annealing from `gate_tau_start` to `gate_tau_end`
- Hardened checkpoints: non-selected LoRA weights zeroed before saving
- Gate metadata stored in manifest (`selected_blocks`, `final_gate_scores`, etc.)
- Compatible with matched forgetting (sweeps over K values instead of ranks)

**Configuration:**
```yaml
unlearn:
  types: [LORA]
  lora_ranks: [8]  # Single rank required for matched forgetting with learned top-K
  types_config:
    LORA:
      layer_selection_mode: "learned_topk_hard"  # Enable learned top-K
      lora_layer_budget_k: 4  # Number of blocks to select (must be ≤ num_layers)
      gate_tau_start: 10.0    # Initial temperature (softer selection)
      gate_tau_end: 0.1       # Final temperature (harder selection)
      gate_warmup_steps: 0    # Steps before annealing begins
      gate_seed: null         # Gate initialization seed (default: uses data_seed)
      gate_reg_coeff: 0.0     # L2 regularization on gate logits
```

**Requirements:**
- `lora_rank > 0` (LoRA must be enabled)
- `lora_layer_budget_k` must be set and > 0
- `lora_layer_budget_k ≤ num_layers`

**Outputs:**
- Checkpoints with K in path: `rank{rank}-k{K}-sc{sc}-{model_id}-...`
- Manifest entries include `selected_blocks`, `final_gate_scores`, gate metadata
- WandB logs: `gating/selected_blocks_step`, `gating/temperature`, `gating/flip_rate`

**Matched Forgetting Integration:**
When matched forgetting is enabled with learned top-K, the pipeline:
- Requires single `lora_rank` (raises error if multiple ranks)
- Sweeps over `lora_layer_budget_k` values (instead of ranks)
- Uses key format `rank{rank}_k{K}` in `matched_forgetting.json`
- Full gate metadata stored and inherited in B checkpoints

**Evidence:** 
- (`unlearn_corpus.py:514-589`) for hook registration
- (`unlearn_corpus.py:1042-1096`) for training loop integration
- (`pipeline.py:2281-2508`) for matched forgetting K-sweep
...
| **LORA (Learned Top-K)** | `UnlearnType.LORA` + `layer_selection_mode="learned_topk_hard"` | LoRA with learned Top-K block selection - exactly K transformer blocks have active LoRA adapters (selected via learned gate logits) | `unlearn_corpus.py` | (`unlearn_corpus.py:514-589`, `unlearn_corpus.py:1042-1096`) |
